...
public class VersionManager : IDisposable
{
    ...
    public virtual void SaveVersions()
    {
        ...
        File.WriteAllText(
            VersionsJsonFilePath,
            Serialize(sortedVersions, new Jso(Jso.Default) { WriteIndented = true })
        );
        var versionsProps = new XDocument(
            ...
            new XElement(
                ...
                new XElement(
                    "PropertyGroup",
                    new XElement(
                        "JustInTimeVersioningVersion",
                        sortedVersions["JustInTimeVersioningSdk"]
                    )
                ),
                new XElement(
                    "ItemGroup",
                    sortedVersions.Select(
                        kvp =>
                            new XElement(
                                "PackageReference",
    ...
    public virtual string VersionsJsonFilePath =>
        Format(
            Combine(
                GetDirectoryNameOfFileAbove(
                    CurrentDirectory.FullName,
                    Format(VersionsJsonFileName, Configuration)
                ),
                VersionsJsonFileName
            ),
            Configuration
        );
    public virtual string VersionsPropsFilePath =>
        Format(
            Combine(
                GetDirectoryNameOfFileAbove(
                    CurrentDirectory.FullName,
                    Format(VersionsPropsFileName, Configuration)
                ),
    ...
    private IStringDictionary InitializeVersionsDictionary()
    {
        ...
        return Deserialize<StringDictionary>(File.ReadAllText(VersionsJsonFilePath))!;
    }
    ...
    public virtual string GetPathOfRootDirectory(
        DirectoryInfo currentDirectory,
        string lookingForDirectoryName,
        DirectoryInfo? lastDirectoryWhereTheThingWasFound = default
    )
    {
        ...
        {
            lastDirectoryWhereTheThingWasFound = new DirectoryInfo(
                currentDirectory.GetFileSystemInfos(lookingForDirectoryName).First().FullName
            );
        }
        return GetPathOfRootDirectory(
            currentDirectory.Parent,
            lookingForDirectoryName,
            currentDirectory
        );
    }
